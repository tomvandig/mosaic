 ...
 *
 * Set once, when the page starts. A server sets one that asks it over http; the static
 * build sets one that holds the archives in memory and answers out of them. Everything
 * below this line asks the same four questions either way.
 */
let source = null;
...
async function loadScene() {
  ...
  try {
    ...
    // The renderer wants a buffer it owns; a source may hand back a view onto a longer one.
    const bytes = answered.buffer.slice(
      answered.byteOffset, answered.byteOffset + answered.byteLength);
    state.glb = bytes;
...
$("file").onchange = async event => {
  ...
  for (const file of files) {
    ...
    try {
      ...
      // Whatever was just added is what you want to look at.
...
/**
 * Draws everything again, from whatever the source says now.
 *
 * Used after an archive has been edited in place: the bytes behind the scene changed, so
 * the glb has to be built again rather than redrawn. What was selected and opened is kept,
 * since the point of an edit is usually to look at the same thing differently.
 */
...
/**
 * Starts the viewer against somewhere to get archives from.
 *
 * Both builds come through here. What differs between them is the argument: a source that
 * asks a server, or one holding archives read out of a folder of static files.
 */
export async function start(from) {
  source = from;

  // Switching to another set of archives is switching to another scene: nothing about
  // what was shown, selected or opened means anything against the new one.
  state.tesserae = [];
  state.shown.clear();
  state.version.clear();
  ...
  // Uploading is the one thing not every source can do. Rather than offering a control
  // that would fail, the page drops it.
